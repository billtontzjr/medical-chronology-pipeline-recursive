...
Evidence protocol v7 restores the complete original source span when a proposed quotation uses explicit ellipses between two to four complete sentences. Each sentence must match uniquely and in order on the same physical page, with at most 2,000 intervening characters per gap. The stored citation and independent audit receive the complete passage, including every omitted qualification. Missing, ambiguous, reordered, altered or partial-sentence fragments remain rejected. Generation also explicitly requests contiguous quotations without ellipses. Earlier saved results remain unchanged until resumed.
